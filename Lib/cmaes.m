...
defopts.DispFinal  = 'off   % display messages like initial and final message';
defopts.DispModulo = '0  % [0:Inf], disp messages after every i-th iteration';
defopts.SaveVariables = 'off   % [on|final|off][-v6] save variables to .mat file';
defopts.SaveFilename = 'variablescmaes.mat  % save all variables, see SaveVariables'; 
defopts.LogModulo = '0    % [0:Inf] if >1 record data less frequently after gen=100';
defopts.LogTime   = '0   % [0:100] max. percentage of time for recording data';
...
if flgresume % resume is on
  ...
  if myevalbool(opts.EvalInitialX)
      %modif
    tmp_hist1=feval(fitfun, xmean', varargin{:}); 
    fitness.hist(1)=tmp_hist1';
      %
...
while isempty(stopflag)
  ...
  if flgEvalParallel
      ...
      % fitness function. arx and arxvalid should not be changed.
       %modif
      tmp_raw = feval(fitfun, arxvalid', varargin{:}); 
      fitness.raw=tmp_raw';
       %
  ...
  for k=find(isnan(fitness.raw)), 
    ...
    while isnan(fitness.raw(k))
      ...
      % fitness function. arx should not be changed.
       %modif
      tmp_raw_k = feval(fitfun, arxvalid(:,k)', varargin{:}); 
      fitness.raw(k) = tmp_raw_k'; 
      tries = tries + 1;
...
if length(stopflag) > sum(strcmp(stopflag, 'stoptoresume')) % final stopping
  ...
  out.solutions.mean.f = tmp_outsolmeanf';
  %
  counteval = counteval + 1;
  out.solutions.mean.evals = counteval;

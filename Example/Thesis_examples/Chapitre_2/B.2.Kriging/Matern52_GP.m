...
N = 3; % Nombre de chemins ï¿½tracer 

mu = 2; % Moyenne
sig2 = 0.5; % Variance
theta1 = 0.1 ; % Hyperparamï¿½tre 1
theta2 = 1 ; % Hyperparamï¿½tre 2
theta3 = 5 ; % Hyperparamï¿½tre 3
...
t1 = sqrt(5).* dist_all_theta1;
t2 = sqrt(5).* dist_all_theta2;
t3 = sqrt(5).* dist_all_theta3;

cov_mat1 = sig2 * ((1+t1+(t1.^2)./3).*exp(-t1) + diag(lambda.*ones(n,1)));
cov_mat2 = sig2 * ((1+t2+(t2.^2)./3).*exp(-t2) + diag(lambda.*ones(n,1)));
cov_mat3 = sig2 * ((1+t3+(t3.^2)./3).*exp(-t3) + diag(lambda.*ones(n,1)));
...
x_R = linspace(0,2,n)'; 
t1 = sqrt(5).* x_R .* theta1;
t2 = sqrt(5).* x_R .* theta2;
t3 = sqrt(5).* x_R .* theta3;

R1 = (1+t1+(t1.^2)./3).*exp(-t1);
R2 = (1+t2+(t2.^2)./3).*exp(-t2);
R3 = (1+t3+(t2.^2)./3).*exp(-t3);

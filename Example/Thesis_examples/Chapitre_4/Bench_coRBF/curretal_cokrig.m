...
for i=1:length(n_HF)
    ...
    for j=1:length(n_LF)
        ...
        for k = 1 : 100
            ...
            % Construction du Cokrigeage
            cokrig_meta = Cokriging(prob,1,[],'corr_HF','corrmatern52','corr_LF','corrmatern52');
            
            % Calcul erreur de prédiction et extraction des hyperparamètres
            RAAE_curretal(i,j,k)  = cokrig_meta.Raae(x_test,y_test);
            
        end
    end
end

save('cokrig_curretal_result','RAAE_curretal')

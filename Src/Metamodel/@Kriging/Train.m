...
if isempty(obj.hyp_corr)
    ...
    if isempty( obj.lb_hyp_corr ) && isempty( obj.ub_hyp_corr )
        ...
        if xor( isempty(obj.lb_hyp_corr), isempty(obj.ub_hyp_corr) )
            warning('Both bounds lb_hyp_corr and ub_hyp_corr have to be defined ! Default values are applied')
        end
        ...
        obj.hyp_corr0 = hyp_corr0_temp * ones(1,obj.prob.m_x);
    else
    ...
    if isempty(obj.hyp_reg)
        ...
        if isempty( obj.lb_hyp_reg ) && isempty( obj.ub_hyp_reg )
            ...
            if xor( isempty(obj.lb_hyp_reg), isempty(obj.ub_hyp_reg) )
                warning('Both bounds lb_reg and ub_reg have to be defined ! Default values are applied')
            end
            obj.lb_reg = 1e-8;
            obj.ub_reg = max( var(obj.f_train), 1e-7 );
        ...
        opts.lambdaBounds = log10([obj.lb_reg ; obj.ub_reg]);
...
obj.hyp_corr_bounds = 10.^[obj.lb_hyp_corr(1) , obj.ub_hyp_corr(1)];
